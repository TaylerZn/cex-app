...
class NetworkLoggerEventScreen extends StatelessWidget {
  ...
  Widget buildRequestView(BuildContext context) {
    print('URL: ${event.request?.uri.toString()}');
    print('Method: ${event.request?.method}');
    print('Timestamp: ${event.timestamp}');
    if (event.request?.headers.isNotEmpty ?? false) {
    ...
    if (event.request?.data != null) {
      if (event.request?.data is Map) {
        print('Request Body (JSON): ${jsonEncode(event.request?.data)}');
      } else if (event.request?.data is String) {
        print('Request Body (String): ${event.request?.data}');
      } else {
        print('Request Body: ${event.request?.data}');
      }
    } else {
  ...
  Widget buildResponseView(BuildContext context) {
    print('Response Status Code: ${event.response?.statusCode}');
    print('Response Status Message: ${event.response?.statusMessage}');
    if (event.response?.headers.isNotEmpty ?? false) {
      print('Response Headers: ${event.response?.headers}');
    }
    // Printing the response body
    if (event.response?.data != null) {
      if (event.response?.data is Map) {
        print('Response Body (JSON): ${jsonEncode(event.response?.data)}');
      } else if (event.response?.data is String) {
        print('Response Body (String): ${event.response?.data}');
      } else {
        print('Response Body: ${event.response?.data}');
      }
    } else {
