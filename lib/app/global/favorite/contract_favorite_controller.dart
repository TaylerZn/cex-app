...
class ContractOptionController extends GetxController {
  static ContractOptionController get to => Get.find();
  ...
  Future<List<String>> fetchOptionContractIdList() async {
    try {
      if (UserGetx.to.isLogin) {
        final res = await ContractApi.instance().getContractOptionalList();
    ...
    if (UserGetx.to.isLogin) {
      try {
        ...
        MarketDataManager.instance
            .updateOptionSymbol(type: MarketSecondType.perpetualContract);
        _isupdating = false;
        return true;
      } catch (e) {
