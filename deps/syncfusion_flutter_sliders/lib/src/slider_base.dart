  ...
  void drawLabelsTicksAndDividers(
    ...
    if (divisions != null && divisions! > 0) {
      while (textValue <= maxRange) {
        double tickPosition = _majorTickPositions[dateTimePos];

        if (dateTimePos == _majorTickPositions.length - 1) {
          tickPosition = _majorTickPositions.last + sliderThemeData.thumbRadius;
        } else if (dateTimePos == 0) {
          tickPosition = _majorTickPositions[0] - sliderThemeData.thumbRadius;
        }
        // Drawing ticks.
        if (_showTicks) {
          _drawTick(
