...
class ChartPainter extends BaseChartPainter {
  ...
  void drawBg(Canvas canvas, Size size) {
    ...
    // 在主图 mainRect 左下角画个 logo图片
    if (chartStyle.showLogo) {
      final logo = ChartStyle.logo;
      if (logo != null) {
        final logoSize = chartStyle.logoSize;
        final logoPadding = chartStyle.logoPadding;
        final logoRect = Rect.fromLTWH(
          (mainRect.width - logoSize.width * 1.5) * 0.5,
          (mainRect.height - logoSize.height * 2) * 0.5,
          logoSize.width * 1.5,
          logoSize.height * 1.5,
        );
...
//    }
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    if (translateXtoX(getX(index)) < mWidth / 2) {
      ...
      canvas.drawRRect(
          ...
          selectPointPaint..color = Colors.black);
      // canvas.drawPath(path, selectPointPaint..color = Colors.black);
    ...
    //         y + baseLine + r),
    //     selectorBorderPaint..color = Colors.black);

    dateTp.paint(canvas, Offset(x - textWidth / 2, y - 4));
    //长按显示这条数据详情
  ...
  void drawNowPrice(Canvas canvas) {
    ...
    nowPricePaint.color = chartColors.nowPriceLineColor;

    //再画背景和文本
    TextSpan span = TextSpan(text:value.toStringAsFixed(fixedLength), style: TextStyle(fontSize: 10.0, color: chartColors.nowPriceTextColor,fontWeight: FontWeight.w500));
    TextPainter tp = TextPainter(text: span, textDirection: TextDirection.ltr);
    tp.layout();

    //先画横线
    double x = translateXtoX(getX((datas?.length ?? 0) - 1));
    double startX = x;
    final max = (mWidth - tp.width - 16) / scaleX;
    ...
    /// 画个白底矩形，边框黑色，圆角6，填充白色
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTRB(
            offsetX - 14, top - 2, offsetX + tp.width - 6, top + tp.height + 2),
    ...
    tp.paint(canvas, Offset(offsetX - 10, top));
  }
...
//For TrendLine
  ...
  void drawCrossLine(Canvas canvas, Size size) {
    ...
    final maxY = size.height - mBottomPadding;
    final space = chartStyle.nowPriceLineSpan + chartStyle.nowPriceLineLength;
    while (startY < maxY) {
      canvas.drawLine(Offset(x, startY),
          Offset(x, startY + chartStyle.nowPriceLineLength), paintY);
      startY += space;
    }
    ...
    final spaceX = chartStyle.nowPriceLineSpan + chartStyle.nowPriceLineLength;
    while (startX < max) {
      canvas.drawLine(Offset(startX, y),
          Offset(startX + chartStyle.nowPriceLineLength, y), paintX);
      startX += spaceX;
    }
